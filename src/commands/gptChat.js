...
module.exports = {
    name: 'gptchat',
    description: 'Chat with the bot using GPT 4.',
    async execute(message) {
        const userMessage = message.content;  // Get the content of the user message
        Option = 'message';  // Set the option to 'message' for text chat
        
        try {
            message.channel.send(await getGPTResponse(userMessage,'message'));//respond with the GPT-generated message
        } catch (error) {
